...
ALLOWED_TYPES = {
    ...
    'enemy', 'helper', 'friend', 'mid-boss', 'boss'
}
...
def process_kirby_data():
  start_time = time.perf_counter()

  # 1. Read files directly in memory
  combined_kirby_data = {}
  combined_kirby_data.update(load_json_file('enemies.json'))
  combined_kirby_data.update(load_json_file('characters.json'))

  final_results = []

  # 2. Process character items synchronously
  for character_key, character_data in combined_kirby_data.items():
